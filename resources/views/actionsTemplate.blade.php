    ...
    @can($gateKey . 'edit')
        ...
        @if (!in_array($controller, $noeditcontrollers))
            ...
            <?php
                if (in_array($controller, array('QuoteTasksController', 'QuotesRemindersController', 'QuotesNotesController'))) {
            ?>
            <a href="{{ route($routeKey . '.edit', ['quote_id' => $row->quote_id, 'id' => $row->id]) }}"
            ...
            <?php        } elseif (in_array($controller, array('InvoiceTasksController', 'InvoiceRemindersController', 'InvoiceNotesController'))) {
                ?>
            <a href="{{ route($routeKey . '.edit', ['invoice_id' => $row->invoice_id, 'id' => $row->id]) }}"
                class="btn btn-xs btn-info"><?php            echo $title; ?></a>
            <?php
...
if ('LanguagesController' === $controller) {
    ...
    @can($gateKey . 'edit')
        ...
        @else
            <a href="{{ url('admin/language/' . $row->code) }}"
                class="btn btn-xs btn-success">@lang('global.app_make_default')</a>
        @endif
    @endcan
    @can($gateKey . 'edit')
        <?php
            ...
            ?>
        <a href="{{ route('admin.language.changedirection', $row->id) }}" class="btn btn-xs btn-success"
            title="{{$title}}">{{$direction}}</a>
    @endcan
    <?php
}
if (in_array($controller, ['ContactsController'])) {
    ...
    @can($gateKey . 'edit')
        <a href="javascript:void(0);" class="btn btn-xs btn-success"
            onclick="sendMail('sendcontactemail', '{{$row->id}}', '{{$row->id}}')">@lang('global.app_send_email')</a>

    @endcan
    @if (!in_array(LEADS_TYPE, $contact_types))
        @if('no' === $row->is_user && Gate::allows('user_create'))
            <a href="{{route('admin.users.create', $row->id)}}"
                class="btn btn-xs btn-warning">@lang('global.app_create_user')</a>
...
if ('CurrenciesController' === $controller) {
    ...
    @can($gateKey . 'edit')
        ...
        @else
            <a href="{{ route('admin.currency.makedefault', $row->id) }}"
                class="btn btn-xs btn-success">@lang('global.app_make_default')</a>
        @endif
    @endcan
    <?php
...
if ('TemplatesController' === $controller) {
    ?>
    @can($gateKey . 'edit')
        <a href="{{ route('admin.templates.duplicate', $row->id) }}"
            class="btn btn-xs btn-success">@lang('others.duplicate')</a>
    @endcan
    <?php
}
if ('ClientProjectsController' === $controller && 'index' === $action) {
    ?>
    @can($gateKey . 'edit')
        <a href="{{ route('admin.client_projects.duplicate', $row->id) }}" class="btn btn-xs btn-success"
            onclick="return confirm(window.are_you_sure_duplicate)">@lang('others.duplicate')</a>
    @endcan
    <?php
}
    ...
    @can($gateKey . 'change_status')
        @if (Auth::id() != $row->id)
            @if ('Active' === $row->status)
                <a href="{{ route('admin.users.changestatus', $row->id) }}"
                    class="btn btn-xs btn-success">@lang('global.users.suspend')</a>
            @else
                <a href="{{ route('admin.users.changestatus', $row->id) }}"
                    class="btn btn-xs btn-success">@lang('global.users.activate')</a>
            @endif
        @endif
    @endcan
    <?php
...
if ('MasterSettingsController' === $controller) {
    ?>
    @can($gateKey . 'edit')
        <a href="{{ url('admin/mastersettings/settings/view', $row->slug) }}"
            class="btn btn-xs btn-info">@lang('global.app_settings')</a>
        @if(env('APP_DEV'))
            <a href="{{ url('admin/mastersettings/settings/add-sub-settings', $row->slug) }}"
                class="btn btn-xs btn-info">@lang('global.app_add_key')</a>
        @endif
    @endcan
    <?php
}
if ('SiteThemesController' === $controller) {
    ...
    @can($gateKey . 'settings')
        @if($row->is_active == 1)
            <a href="{{ url('admin/make/default/theme', $row->id) }}"
                class="btn btn-xs btn-success">@lang('global.app_default')</a>
        @else
            <a href="{{ url('admin/make/default/theme', $row->id) }}"
                class="btn btn-xs btn-primary">@lang('global.app_make_default')</a>
        @endif
        @if(!isDemo())
            <a href="{{ url('admin/theme/settings', $row->slug) }}" class="btn btn-xs btn-info">@lang('global.app_settings')</a>
            <a href="{{ url('admin/theme/settings/add-sub-settings', $row->slug) }}"
                class="btn btn-xs btn-info">@lang('global.app_add_key')</a>
        @endif
    @endcan
    <?php
...
if ('ModulesManagementsController' === $controller) {
    ...
    <a href="{{ route('admin.modules-management.changestatus', $row->id) }}"
        class="btn btn-xs btn-{{$class}} @if('no' === $row->can_inactive) disabled @endif" title="{{$title}}" @if('no' === $row->can_inactive) onclick="return false;" disabled @endif>{{$label}}</a>


    @if(!isDemo() && 'Custom' === $row->type)
        <a href="{{ url('admin/plugin/settings', $row->slug) }}"
            class="btn btn-xs btn-info">@lang('global.app_settings')</a>
        <a href="{{ url('admin/plugin/settings/add-sub-settings', $row->slug) }}"
            class="btn btn-xs btn-info">@lang('global.app_add_key')</a>
    @endif
    <?php
        //}
    ...
    ?>
    @can($gateKey . 'receipt')
        <a href="{{ route('admin.incomes.receipt', $row->slug) }}"
            class="btn btn-xs btn-success">@lang('custom.incomes.receipt')</a>
    @endcan
    <?php
}
if ('OrdersController' === $controller) {
    ...
    @can('order_cancel')
        @if (in_array($row->status, array('Pending', 'Cancelled', 'Returned')))
            ...
            <div class="btn-group ">
                <button type="button" class="btn btn-success mb-1 btn-min-width dropdown-toggle" data-toggle="dropdown"
                    aria-haspopup="true" aria-expanded="false">
                    <i class="fa fa-arrows-v" aria-hidden="true"></i>&nbsp;{{trans('custom.common.mark-as')}}&nbsp;<span
                        class="caret"></span>
                </button>
                <div class="dropdown-menu">
                    <li><a class="dropdown-item"
                            href="{{ route('admin.orders.cancel', ['slug' => $row->slug, 'status' => 'Cancelled']) }}">{{trans('custom.orders.cancel')}}</a>
                    </li>

                    <li><a class="dropdown-item"
                            href="{{ route('admin.orders.cancel', ['slug' => $row->slug, 'status' => 'Active']) }}">{{trans('custom.orders.active')}}</a>
                    </li>

                    <li><a class="dropdown-item"
                            href="{{ route('admin.orders.cancel', ['slug' => $row->slug, 'status' => 'Pending']) }}">{{trans('custom.orders.pending')}}</a>
                    </li>

                    <li><a class="dropdown-item"
                            href="{{ route('admin.orders.cancel', ['slug' => $row->slug, 'status' => 'Returned']) }}">{{trans('custom.orders.return')}}</a>
                    </li>
            ...
            <?php
                    if (isCustomer()) {
                        ...
                        if (!$isExpired) {
                    ?>
            <a href="{{ route('admin.orders.payment-now', $row->slug) }}" class="btn btn-xs btn-info"
                onclick="return confirm('{{trans("orders::global.orders.app_are_you_sure")}}')">@lang('orders::global.orders.retry')</a>
            <?php                }
                    }
                    ?>
    ...
    @endcan
    @if(isCustomer())
        <a href="{{ route('admin.orders.reorder', $row->slug) }}" class="btn btn-xs btn-info"
            onclick="return confirm('{{trans("orders::global.orders.app_are_you_sure_reorder")}}')">@lang('orders::global.orders.reorder')</a>
    @endif
    <?php
