...
if ( 'LanguagesController' === $controller ) {
    ...
    @can($gateKey.'edit')
        ...
        &nbsp;|&nbsp;<p class="primary" data-toggle="tooltip" data-placement="bottom" data-original-title="{{trans('global.app_default')}}">@if( 'button' === $addnew_type ) @lang('global.app_default') @else <i class="fa fa-pencil-square-o" aria-hidden="true"></i> @endif</p>
        @else
        &nbsp;|&nbsp;<a href="{{ url('admin/language/' . $row->code) }}" class="success" data-toggle="tooltip" data-placement="bottom" data-original-title="{{trans('global.app_make_default')}}">@if( 'button' === $addnew_type ) @lang('global.app_make_default') @else <i class="fa fa-pencil-square-o" aria-hidden="true"></i> @endif</a>
        @endif
    @endcan
    @can($gateKey.'edit')
        <?php
...
@can($gateKey.'delete')
    ...
    if ( 'ContactsController' === $controller ) {
        echo '&nbsp;|&nbsp;<a href="'. route('admin.contacts.info', $row->id) .'" class="danger">'.trans('global.app_delete') . '</a>';
    } else {
    ?>&nbsp;|&nbsp;
    {!! Form::open(array(
        ...
        @else
            <input class="danger" style="border: 0px; color: red; background: none;" type="submit" value=""  data-toggle="tooltip" data-placement="bottom" data-original-title="{{trans('global.app_delete')}}"><i class="fa fa-trash"></i></input> 
        @endif
    {!! Form::close() !!}
    <?php } ?>
@endcan
